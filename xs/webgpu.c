SV *WGPUAdapter__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::Adapter", GV_ADD));
  return RETVAL;
}

SV *WGPUBindGroup__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::BindGroup", GV_ADD));
  return RETVAL;
}

SV *WGPUBindGroupLayout__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::BindGroupLayout", GV_ADD));
  return RETVAL;
}

SV *WGPUBuffer__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::Buffer", GV_ADD));
  return RETVAL;
}

SV *WGPUCommandBuffer__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::CommandBuffer", GV_ADD));
  return RETVAL;
}

SV *WGPUCommandEncoder__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::CommandEncoder", GV_ADD));
  return RETVAL;
}

SV *WGPUComputePassEncoder__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::ComputePassEncoder", GV_ADD));
  return RETVAL;
}

SV *WGPUComputePipeline__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::ComputePipeline", GV_ADD));
  return RETVAL;
}

SV *WGPUDevice__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::Device", GV_ADD));
  return RETVAL;
}

SV *WGPUInstance__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::Instance", GV_ADD));
  return RETVAL;
}

SV *WGPUPipelineLayout__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::PipelineLayout", GV_ADD));
  return RETVAL;
}

SV *WGPUQuerySet__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::QuerySet", GV_ADD));
  return RETVAL;
}

SV *WGPUQueue__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::Queue", GV_ADD));
  return RETVAL;
}

SV *WGPURenderBundle__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::RenderBundle", GV_ADD));
  return RETVAL;
}

SV *WGPURenderBundleEncoder__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::RenderBundleEncoder", GV_ADD));
  return RETVAL;
}

SV *WGPURenderPassEncoder__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::RenderPassEncoder", GV_ADD));
  return RETVAL;
}

SV *WGPURenderPipeline__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::RenderPipeline", GV_ADD));
  return RETVAL;
}

SV *WGPUSampler__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::Sampler", GV_ADD));
  return RETVAL;
}

SV *WGPUShaderModule__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::ShaderModule", GV_ADD));
  return RETVAL;
}

SV *WGPUSurface__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::Surface", GV_ADD));
  return RETVAL;
}

SV *WGPUSwapChain__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::SwapChain", GV_ADD));
  return RETVAL;
}

SV *WGPUTexture__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::Texture", GV_ADD));
  return RETVAL;
}

SV *WGPUTextureView__wrap( void *n )
{
  HV *h = newHV();
  SV *RETVAL = sv_2mortal(newRV((SV*)h));

  sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  sv_bless(RETVAL, gv_stashpv("WebGPU::Direct::TextureView", GV_ADD));
  return RETVAL;
}

void WGPUChainedStruct__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ChainedStruct"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ChainedStruct",
      "THIS", "WebGPU::Direct::ChainedStruct");
  }

  HV *h = SvRV(THIS);
  WGPUChainedStruct *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUChainedStruct);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "next", 4, &n->next, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_enum(aTHX_ h, "sType", 5, &n->sType, newSVpvs("WGPUSType"));

}

void WGPUChainedStructOut__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ChainedStructOut"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ChainedStructOut",
      "THIS", "WebGPU::Direct::ChainedStructOut");
  }

  HV *h = SvRV(THIS);
  WGPUChainedStructOut *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUChainedStructOut);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "next", 4, &n->next, newSVpvs("WebGPU::Direct::ChainedStructOut"));
  _find_set_enum(aTHX_ h, "sType", 5, &n->sType, newSVpvs("WGPUSType"));

}

void WGPUAdapterProperties__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::AdapterProperties"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::AdapterProperties",
      "THIS", "WebGPU::Direct::AdapterProperties");
  }

  HV *h = SvRV(THIS);
  WGPUAdapterProperties *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUAdapterProperties);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStructOut"));
  _find_set_uint32_t(aTHX_ h, "vendorID", 8, &n->vendorID, newSVpvs("uint32_t"));
  _find_set_str(aTHX_ h, "vendorName", 10, &n->vendorName, newSVpvs("char"));
  _find_set_str(aTHX_ h, "architecture", 12, &n->architecture, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "deviceID", 8, &n->deviceID, newSVpvs("uint32_t"));
  _find_set_str(aTHX_ h, "name", 4, &n->name, newSVpvs("char"));
  _find_set_str(aTHX_ h, "driverDescription", 17, &n->driverDescription, newSVpvs("char"));
  _find_set_enum(aTHX_ h, "adapterType", 11, &n->adapterType, newSVpvs("WGPUAdapterType"));
  _find_set_enum(aTHX_ h, "backendType", 11, &n->backendType, newSVpvs("WGPUBackendType"));

}

void WGPUBindGroupEntry__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::BindGroupEntry"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::BindGroupEntry",
      "THIS", "WebGPU::Direct::BindGroupEntry");
  }

  HV *h = SvRV(THIS);
  WGPUBindGroupEntry *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUBindGroupEntry);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_uint32_t(aTHX_ h, "binding", 7, &n->binding, newSVpvs("uint32_t"));
  _find_set_void(aTHX_ h, "buffer", 6, &n->buffer, newSVpvs("WGPUBuffer"));
  _find_set_uint64_t(aTHX_ h, "offset", 6, &n->offset, newSVpvs("uint64_t"));
  _find_set_uint64_t(aTHX_ h, "size", 4, &n->size, newSVpvs("uint64_t"));
  _find_set_void(aTHX_ h, "sampler", 7, &n->sampler, newSVpvs("WGPUSampler"));
  _find_set_void(aTHX_ h, "textureView", 11, &n->textureView, newSVpvs("WGPUTextureView"));

}

void WGPUBlendComponent__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::BlendComponent"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::BlendComponent",
      "THIS", "WebGPU::Direct::BlendComponent");
  }

  HV *h = SvRV(THIS);
  WGPUBlendComponent *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUBlendComponent);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_enum(aTHX_ h, "operation", 9, &n->operation, newSVpvs("WGPUBlendOperation"));
  _find_set_enum(aTHX_ h, "srcFactor", 9, &n->srcFactor, newSVpvs("WGPUBlendFactor"));
  _find_set_enum(aTHX_ h, "dstFactor", 9, &n->dstFactor, newSVpvs("WGPUBlendFactor"));

}

void WGPUBufferBindingLayout__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::BufferBindingLayout"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::BufferBindingLayout",
      "THIS", "WebGPU::Direct::BufferBindingLayout");
  }

  HV *h = SvRV(THIS);
  WGPUBufferBindingLayout *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUBufferBindingLayout);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_enum(aTHX_ h, "type", 4, &n->type, newSVpvs("WGPUBufferBindingType"));
  _find_set_bool(aTHX_ h, "hasDynamicOffset", 16, &n->hasDynamicOffset, newSVpvs("bool"));
  _find_set_uint64_t(aTHX_ h, "minBindingSize", 14, &n->minBindingSize, newSVpvs("uint64_t"));

}

void WGPUBufferDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::BufferDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::BufferDescriptor",
      "THIS", "WebGPU::Direct::BufferDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUBufferDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUBufferDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_WGPUFlags(aTHX_ h, "usage", 5, &n->usage, newSVpvs("WGPUBufferUsageFlags"));
  _find_set_uint64_t(aTHX_ h, "size", 4, &n->size, newSVpvs("uint64_t"));
  _find_set_bool(aTHX_ h, "mappedAtCreation", 16, &n->mappedAtCreation, newSVpvs("bool"));

}

void WGPUColor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::Color"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::Color",
      "THIS", "WebGPU::Direct::Color");
  }

  HV *h = SvRV(THIS);
  WGPUColor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUColor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_double(aTHX_ h, "r", 1, &n->r, newSVpvs("double"));
  _find_set_double(aTHX_ h, "g", 1, &n->g, newSVpvs("double"));
  _find_set_double(aTHX_ h, "b", 1, &n->b, newSVpvs("double"));
  _find_set_double(aTHX_ h, "a", 1, &n->a, newSVpvs("double"));

}

void WGPUCommandBufferDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::CommandBufferDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::CommandBufferDescriptor",
      "THIS", "WebGPU::Direct::CommandBufferDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUCommandBufferDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUCommandBufferDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));

}

void WGPUCommandEncoderDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::CommandEncoderDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::CommandEncoderDescriptor",
      "THIS", "WebGPU::Direct::CommandEncoderDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUCommandEncoderDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUCommandEncoderDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));

}

void WGPUCompilationMessage__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::CompilationMessage"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::CompilationMessage",
      "THIS", "WebGPU::Direct::CompilationMessage");
  }

  HV *h = SvRV(THIS);
  WGPUCompilationMessage *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUCompilationMessage);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "message", 7, &n->message, newSVpvs("char"));
  _find_set_enum(aTHX_ h, "type", 4, &n->type, newSVpvs("WGPUCompilationMessageType"));
  _find_set_uint64_t(aTHX_ h, "lineNum", 7, &n->lineNum, newSVpvs("uint64_t"));
  _find_set_uint64_t(aTHX_ h, "linePos", 7, &n->linePos, newSVpvs("uint64_t"));
  _find_set_uint64_t(aTHX_ h, "offset", 6, &n->offset, newSVpvs("uint64_t"));
  _find_set_uint64_t(aTHX_ h, "length", 6, &n->length, newSVpvs("uint64_t"));
  _find_set_uint64_t(aTHX_ h, "utf16LinePos", 12, &n->utf16LinePos, newSVpvs("uint64_t"));
  _find_set_uint64_t(aTHX_ h, "utf16Offset", 11, &n->utf16Offset, newSVpvs("uint64_t"));
  _find_set_uint64_t(aTHX_ h, "utf16Length", 11, &n->utf16Length, newSVpvs("uint64_t"));

}

void WGPUComputePassTimestampWrite__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ComputePassTimestampWrite"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ComputePassTimestampWrite",
      "THIS", "WebGPU::Direct::ComputePassTimestampWrite");
  }

  HV *h = SvRV(THIS);
  WGPUComputePassTimestampWrite *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUComputePassTimestampWrite);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_void(aTHX_ h, "querySet", 8, &n->querySet, newSVpvs("WGPUQuerySet"));
  _find_set_uint32_t(aTHX_ h, "queryIndex", 10, &n->queryIndex, newSVpvs("uint32_t"));
  _find_set_enum(aTHX_ h, "location", 8, &n->location, newSVpvs("WGPUComputePassTimestampLocation"));

}

void WGPUConstantEntry__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ConstantEntry"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ConstantEntry",
      "THIS", "WebGPU::Direct::ConstantEntry");
  }

  HV *h = SvRV(THIS);
  WGPUConstantEntry *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUConstantEntry);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "key", 3, &n->key, newSVpvs("char"));
  _find_set_double(aTHX_ h, "value", 5, &n->value, newSVpvs("double"));

}

void WGPUExtent3D__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::Extent3D"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::Extent3D",
      "THIS", "WebGPU::Direct::Extent3D");
  }

  HV *h = SvRV(THIS);
  WGPUExtent3D *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUExtent3D);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_uint32_t(aTHX_ h, "width", 5, &n->width, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "height", 6, &n->height, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "depthOrArrayLayers", 18, &n->depthOrArrayLayers, newSVpvs("uint32_t"));

}

void WGPUInstanceDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::InstanceDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::InstanceDescriptor",
      "THIS", "WebGPU::Direct::InstanceDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUInstanceDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUInstanceDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));

}

void WGPULimits__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::Limits"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::Limits",
      "THIS", "WebGPU::Direct::Limits");
  }

  HV *h = SvRV(THIS);
  WGPULimits *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPULimits);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_uint32_t(aTHX_ h, "maxTextureDimension1D", 21, &n->maxTextureDimension1D, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxTextureDimension2D", 21, &n->maxTextureDimension2D, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxTextureDimension3D", 21, &n->maxTextureDimension3D, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxTextureArrayLayers", 21, &n->maxTextureArrayLayers, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxBindGroups", 13, &n->maxBindGroups, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxBindingsPerBindGroup", 23, &n->maxBindingsPerBindGroup, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxDynamicUniformBuffersPerPipelineLayout", 41, &n->maxDynamicUniformBuffersPerPipelineLayout, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxDynamicStorageBuffersPerPipelineLayout", 41, &n->maxDynamicStorageBuffersPerPipelineLayout, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxSampledTexturesPerShaderStage", 32, &n->maxSampledTexturesPerShaderStage, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxSamplersPerShaderStage", 25, &n->maxSamplersPerShaderStage, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxStorageBuffersPerShaderStage", 31, &n->maxStorageBuffersPerShaderStage, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxStorageTexturesPerShaderStage", 32, &n->maxStorageTexturesPerShaderStage, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxUniformBuffersPerShaderStage", 31, &n->maxUniformBuffersPerShaderStage, newSVpvs("uint32_t"));
  _find_set_uint64_t(aTHX_ h, "maxUniformBufferBindingSize", 27, &n->maxUniformBufferBindingSize, newSVpvs("uint64_t"));
  _find_set_uint64_t(aTHX_ h, "maxStorageBufferBindingSize", 27, &n->maxStorageBufferBindingSize, newSVpvs("uint64_t"));
  _find_set_uint32_t(aTHX_ h, "minUniformBufferOffsetAlignment", 31, &n->minUniformBufferOffsetAlignment, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "minStorageBufferOffsetAlignment", 31, &n->minStorageBufferOffsetAlignment, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxVertexBuffers", 16, &n->maxVertexBuffers, newSVpvs("uint32_t"));
  _find_set_uint64_t(aTHX_ h, "maxBufferSize", 13, &n->maxBufferSize, newSVpvs("uint64_t"));
  _find_set_uint32_t(aTHX_ h, "maxVertexAttributes", 19, &n->maxVertexAttributes, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxVertexBufferArrayStride", 26, &n->maxVertexBufferArrayStride, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxInterStageShaderComponents", 29, &n->maxInterStageShaderComponents, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxInterStageShaderVariables", 28, &n->maxInterStageShaderVariables, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxColorAttachments", 19, &n->maxColorAttachments, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxColorAttachmentBytesPerSample", 32, &n->maxColorAttachmentBytesPerSample, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxComputeWorkgroupStorageSize", 30, &n->maxComputeWorkgroupStorageSize, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxComputeInvocationsPerWorkgroup", 33, &n->maxComputeInvocationsPerWorkgroup, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxComputeWorkgroupSizeX", 24, &n->maxComputeWorkgroupSizeX, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxComputeWorkgroupSizeY", 24, &n->maxComputeWorkgroupSizeY, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxComputeWorkgroupSizeZ", 24, &n->maxComputeWorkgroupSizeZ, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "maxComputeWorkgroupsPerDimension", 32, &n->maxComputeWorkgroupsPerDimension, newSVpvs("uint32_t"));

}

void WGPUMultisampleState__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::MultisampleState"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::MultisampleState",
      "THIS", "WebGPU::Direct::MultisampleState");
  }

  HV *h = SvRV(THIS);
  WGPUMultisampleState *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUMultisampleState);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_uint32_t(aTHX_ h, "count", 5, &n->count, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "mask", 4, &n->mask, newSVpvs("uint32_t"));
  _find_set_bool(aTHX_ h, "alphaToCoverageEnabled", 22, &n->alphaToCoverageEnabled, newSVpvs("bool"));

}

void WGPUOrigin3D__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::Origin3D"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::Origin3D",
      "THIS", "WebGPU::Direct::Origin3D");
  }

  HV *h = SvRV(THIS);
  WGPUOrigin3D *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUOrigin3D);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_uint32_t(aTHX_ h, "x", 1, &n->x, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "y", 1, &n->y, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "z", 1, &n->z, newSVpvs("uint32_t"));

}

void WGPUPipelineLayoutDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::PipelineLayoutDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::PipelineLayoutDescriptor",
      "THIS", "WebGPU::Direct::PipelineLayoutDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUPipelineLayoutDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUPipelineLayoutDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "bindGroupLayoutCount", 20, &n->bindGroupLayoutCount, newSVpvs("uint32_t"));
  _find_set_void(aTHX_ h, "bindGroupLayouts", 16, &n->bindGroupLayouts, newSVpvs("WGPUBindGroupLayout"));

}

void WGPUPrimitiveDepthClipControl__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::PrimitiveDepthClipControl"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::PrimitiveDepthClipControl",
      "THIS", "WebGPU::Direct::PrimitiveDepthClipControl");
  }

  HV *h = SvRV(THIS);
  WGPUPrimitiveDepthClipControl *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUPrimitiveDepthClipControl);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_bool(aTHX_ h, "unclippedDepth", 14, &n->unclippedDepth, newSVpvs("bool"));

}

void WGPUPrimitiveState__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::PrimitiveState"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::PrimitiveState",
      "THIS", "WebGPU::Direct::PrimitiveState");
  }

  HV *h = SvRV(THIS);
  WGPUPrimitiveState *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUPrimitiveState);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_enum(aTHX_ h, "topology", 8, &n->topology, newSVpvs("WGPUPrimitiveTopology"));
  _find_set_enum(aTHX_ h, "stripIndexFormat", 16, &n->stripIndexFormat, newSVpvs("WGPUIndexFormat"));
  _find_set_enum(aTHX_ h, "frontFace", 9, &n->frontFace, newSVpvs("WGPUFrontFace"));
  _find_set_enum(aTHX_ h, "cullMode", 8, &n->cullMode, newSVpvs("WGPUCullMode"));

}

void WGPUQuerySetDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::QuerySetDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::QuerySetDescriptor",
      "THIS", "WebGPU::Direct::QuerySetDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUQuerySetDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUQuerySetDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_enum(aTHX_ h, "type", 4, &n->type, newSVpvs("WGPUQueryType"));
  _find_set_uint32_t(aTHX_ h, "count", 5, &n->count, newSVpvs("uint32_t"));
  _find_set_enum(aTHX_ h, "pipelineStatistics", 18, &n->pipelineStatistics, newSVpvs("WGPUPipelineStatisticName"));
  _find_set_uint32_t(aTHX_ h, "pipelineStatisticsCount", 23, &n->pipelineStatisticsCount, newSVpvs("uint32_t"));

}

void WGPUQueueDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::QueueDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::QueueDescriptor",
      "THIS", "WebGPU::Direct::QueueDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUQueueDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUQueueDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));

}

void WGPURenderBundleDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RenderBundleDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RenderBundleDescriptor",
      "THIS", "WebGPU::Direct::RenderBundleDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPURenderBundleDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURenderBundleDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));

}

void WGPURenderBundleEncoderDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RenderBundleEncoderDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RenderBundleEncoderDescriptor",
      "THIS", "WebGPU::Direct::RenderBundleEncoderDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPURenderBundleEncoderDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURenderBundleEncoderDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "colorFormatsCount", 17, &n->colorFormatsCount, newSVpvs("uint32_t"));
  _find_set_enum(aTHX_ h, "colorFormats", 12, &n->colorFormats, newSVpvs("WGPUTextureFormat"));
  _find_set_enum(aTHX_ h, "depthStencilFormat", 18, &n->depthStencilFormat, newSVpvs("WGPUTextureFormat"));
  _find_set_uint32_t(aTHX_ h, "sampleCount", 11, &n->sampleCount, newSVpvs("uint32_t"));
  _find_set_bool(aTHX_ h, "depthReadOnly", 13, &n->depthReadOnly, newSVpvs("bool"));
  _find_set_bool(aTHX_ h, "stencilReadOnly", 15, &n->stencilReadOnly, newSVpvs("bool"));

}

void WGPURenderPassDepthStencilAttachment__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RenderPassDepthStencilAttachment"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RenderPassDepthStencilAttachment",
      "THIS", "WebGPU::Direct::RenderPassDepthStencilAttachment");
  }

  HV *h = SvRV(THIS);
  WGPURenderPassDepthStencilAttachment *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURenderPassDepthStencilAttachment);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_void(aTHX_ h, "view", 4, &n->view, newSVpvs("WGPUTextureView"));
  _find_set_enum(aTHX_ h, "depthLoadOp", 11, &n->depthLoadOp, newSVpvs("WGPULoadOp"));
  _find_set_enum(aTHX_ h, "depthStoreOp", 12, &n->depthStoreOp, newSVpvs("WGPUStoreOp"));
  _find_set_float(aTHX_ h, "depthClearValue", 15, &n->depthClearValue, newSVpvs("float"));
  _find_set_bool(aTHX_ h, "depthReadOnly", 13, &n->depthReadOnly, newSVpvs("bool"));
  _find_set_enum(aTHX_ h, "stencilLoadOp", 13, &n->stencilLoadOp, newSVpvs("WGPULoadOp"));
  _find_set_enum(aTHX_ h, "stencilStoreOp", 14, &n->stencilStoreOp, newSVpvs("WGPUStoreOp"));
  _find_set_uint32_t(aTHX_ h, "stencilClearValue", 17, &n->stencilClearValue, newSVpvs("uint32_t"));
  _find_set_bool(aTHX_ h, "stencilReadOnly", 15, &n->stencilReadOnly, newSVpvs("bool"));

}

void WGPURenderPassDescriptorMaxDrawCount__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RenderPassDescriptorMaxDrawCount"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RenderPassDescriptorMaxDrawCount",
      "THIS", "WebGPU::Direct::RenderPassDescriptorMaxDrawCount");
  }

  HV *h = SvRV(THIS);
  WGPURenderPassDescriptorMaxDrawCount *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURenderPassDescriptorMaxDrawCount);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_uint64_t(aTHX_ h, "maxDrawCount", 12, &n->maxDrawCount, newSVpvs("uint64_t"));

}

void WGPURenderPassTimestampWrite__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RenderPassTimestampWrite"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RenderPassTimestampWrite",
      "THIS", "WebGPU::Direct::RenderPassTimestampWrite");
  }

  HV *h = SvRV(THIS);
  WGPURenderPassTimestampWrite *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURenderPassTimestampWrite);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_void(aTHX_ h, "querySet", 8, &n->querySet, newSVpvs("WGPUQuerySet"));
  _find_set_uint32_t(aTHX_ h, "queryIndex", 10, &n->queryIndex, newSVpvs("uint32_t"));
  _find_set_enum(aTHX_ h, "location", 8, &n->location, newSVpvs("WGPURenderPassTimestampLocation"));

}

void WGPURequestAdapterOptions__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RequestAdapterOptions"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RequestAdapterOptions",
      "THIS", "WebGPU::Direct::RequestAdapterOptions");
  }

  HV *h = SvRV(THIS);
  WGPURequestAdapterOptions *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURequestAdapterOptions);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "compatibleSurface", 17, &n->compatibleSurface, newSVpvs("WGPUSurface"));
  _find_set_enum(aTHX_ h, "powerPreference", 15, &n->powerPreference, newSVpvs("WGPUPowerPreference"));
  _find_set_bool(aTHX_ h, "forceFallbackAdapter", 20, &n->forceFallbackAdapter, newSVpvs("bool"));

}

void WGPUSamplerBindingLayout__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SamplerBindingLayout"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SamplerBindingLayout",
      "THIS", "WebGPU::Direct::SamplerBindingLayout");
  }

  HV *h = SvRV(THIS);
  WGPUSamplerBindingLayout *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSamplerBindingLayout);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_enum(aTHX_ h, "type", 4, &n->type, newSVpvs("WGPUSamplerBindingType"));

}

void WGPUSamplerDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SamplerDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SamplerDescriptor",
      "THIS", "WebGPU::Direct::SamplerDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUSamplerDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSamplerDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_enum(aTHX_ h, "addressModeU", 12, &n->addressModeU, newSVpvs("WGPUAddressMode"));
  _find_set_enum(aTHX_ h, "addressModeV", 12, &n->addressModeV, newSVpvs("WGPUAddressMode"));
  _find_set_enum(aTHX_ h, "addressModeW", 12, &n->addressModeW, newSVpvs("WGPUAddressMode"));
  _find_set_enum(aTHX_ h, "magFilter", 9, &n->magFilter, newSVpvs("WGPUFilterMode"));
  _find_set_enum(aTHX_ h, "minFilter", 9, &n->minFilter, newSVpvs("WGPUFilterMode"));
  _find_set_enum(aTHX_ h, "mipmapFilter", 12, &n->mipmapFilter, newSVpvs("WGPUMipmapFilterMode"));
  _find_set_float(aTHX_ h, "lodMinClamp", 11, &n->lodMinClamp, newSVpvs("float"));
  _find_set_float(aTHX_ h, "lodMaxClamp", 11, &n->lodMaxClamp, newSVpvs("float"));
  _find_set_enum(aTHX_ h, "compare", 7, &n->compare, newSVpvs("WGPUCompareFunction"));
  _find_set_uint16_t(aTHX_ h, "maxAnisotropy", 13, &n->maxAnisotropy, newSVpvs("uint16_t"));

}

void WGPUShaderModuleCompilationHint__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ShaderModuleCompilationHint"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ShaderModuleCompilationHint",
      "THIS", "WebGPU::Direct::ShaderModuleCompilationHint");
  }

  HV *h = SvRV(THIS);
  WGPUShaderModuleCompilationHint *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUShaderModuleCompilationHint);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "entryPoint", 10, &n->entryPoint, newSVpvs("char"));
  _find_set_void(aTHX_ h, "layout", 6, &n->layout, newSVpvs("WGPUPipelineLayout"));

}

void WGPUShaderModuleSPIRVDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ShaderModuleSPIRVDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ShaderModuleSPIRVDescriptor",
      "THIS", "WebGPU::Direct::ShaderModuleSPIRVDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUShaderModuleSPIRVDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUShaderModuleSPIRVDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_uint32_t(aTHX_ h, "codeSize", 8, &n->codeSize, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "code", 4, &n->code, newSVpvs("uint32_t"));

}

void WGPUShaderModuleWGSLDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ShaderModuleWGSLDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ShaderModuleWGSLDescriptor",
      "THIS", "WebGPU::Direct::ShaderModuleWGSLDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUShaderModuleWGSLDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUShaderModuleWGSLDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "code", 4, &n->code, newSVpvs("char"));

}

void WGPUStencilFaceState__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::StencilFaceState"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::StencilFaceState",
      "THIS", "WebGPU::Direct::StencilFaceState");
  }

  HV *h = SvRV(THIS);
  WGPUStencilFaceState *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUStencilFaceState);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_enum(aTHX_ h, "compare", 7, &n->compare, newSVpvs("WGPUCompareFunction"));
  _find_set_enum(aTHX_ h, "failOp", 6, &n->failOp, newSVpvs("WGPUStencilOperation"));
  _find_set_enum(aTHX_ h, "depthFailOp", 11, &n->depthFailOp, newSVpvs("WGPUStencilOperation"));
  _find_set_enum(aTHX_ h, "passOp", 6, &n->passOp, newSVpvs("WGPUStencilOperation"));

}

void WGPUStorageTextureBindingLayout__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::StorageTextureBindingLayout"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::StorageTextureBindingLayout",
      "THIS", "WebGPU::Direct::StorageTextureBindingLayout");
  }

  HV *h = SvRV(THIS);
  WGPUStorageTextureBindingLayout *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUStorageTextureBindingLayout);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_enum(aTHX_ h, "access", 6, &n->access, newSVpvs("WGPUStorageTextureAccess"));
  _find_set_enum(aTHX_ h, "format", 6, &n->format, newSVpvs("WGPUTextureFormat"));
  _find_set_enum(aTHX_ h, "viewDimension", 13, &n->viewDimension, newSVpvs("WGPUTextureViewDimension"));

}

void WGPUSurfaceDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SurfaceDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SurfaceDescriptor",
      "THIS", "WebGPU::Direct::SurfaceDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUSurfaceDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSurfaceDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));

}

void WGPUSurfaceDescriptorFromAndroidNativeWindow__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SurfaceDescriptorFromAndroidNativeWindow"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SurfaceDescriptorFromAndroidNativeWindow",
      "THIS", "WebGPU::Direct::SurfaceDescriptorFromAndroidNativeWindow");
  }

  HV *h = SvRV(THIS);
  WGPUSurfaceDescriptorFromAndroidNativeWindow *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSurfaceDescriptorFromAndroidNativeWindow);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "window", 6, &n->window, newSVpvs("void"));

}

void WGPUSurfaceDescriptorFromCanvasHTMLSelector__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SurfaceDescriptorFromCanvasHTMLSelector"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SurfaceDescriptorFromCanvasHTMLSelector",
      "THIS", "WebGPU::Direct::SurfaceDescriptorFromCanvasHTMLSelector");
  }

  HV *h = SvRV(THIS);
  WGPUSurfaceDescriptorFromCanvasHTMLSelector *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSurfaceDescriptorFromCanvasHTMLSelector);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "selector", 8, &n->selector, newSVpvs("char"));

}

void WGPUSurfaceDescriptorFromMetalLayer__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SurfaceDescriptorFromMetalLayer"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SurfaceDescriptorFromMetalLayer",
      "THIS", "WebGPU::Direct::SurfaceDescriptorFromMetalLayer");
  }

  HV *h = SvRV(THIS);
  WGPUSurfaceDescriptorFromMetalLayer *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSurfaceDescriptorFromMetalLayer);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "layer", 5, &n->layer, newSVpvs("void"));

}

void WGPUSurfaceDescriptorFromWaylandSurface__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SurfaceDescriptorFromWaylandSurface"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SurfaceDescriptorFromWaylandSurface",
      "THIS", "WebGPU::Direct::SurfaceDescriptorFromWaylandSurface");
  }

  HV *h = SvRV(THIS);
  WGPUSurfaceDescriptorFromWaylandSurface *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSurfaceDescriptorFromWaylandSurface);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "display", 7, &n->display, newSVpvs("void"));
  _find_set_void(aTHX_ h, "surface", 7, &n->surface, newSVpvs("void"));

}

void WGPUSurfaceDescriptorFromWindowsHWND__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SurfaceDescriptorFromWindowsHWND"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SurfaceDescriptorFromWindowsHWND",
      "THIS", "WebGPU::Direct::SurfaceDescriptorFromWindowsHWND");
  }

  HV *h = SvRV(THIS);
  WGPUSurfaceDescriptorFromWindowsHWND *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSurfaceDescriptorFromWindowsHWND);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "hinstance", 9, &n->hinstance, newSVpvs("void"));
  _find_set_void(aTHX_ h, "hwnd", 4, &n->hwnd, newSVpvs("void"));

}

void WGPUSurfaceDescriptorFromXcbWindow__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SurfaceDescriptorFromXcbWindow"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SurfaceDescriptorFromXcbWindow",
      "THIS", "WebGPU::Direct::SurfaceDescriptorFromXcbWindow");
  }

  HV *h = SvRV(THIS);
  WGPUSurfaceDescriptorFromXcbWindow *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSurfaceDescriptorFromXcbWindow);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "connection", 10, &n->connection, newSVpvs("void"));
  _find_set_uint32_t(aTHX_ h, "window", 6, &n->window, newSVpvs("uint32_t"));

}

void WGPUSurfaceDescriptorFromXlibWindow__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SurfaceDescriptorFromXlibWindow"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SurfaceDescriptorFromXlibWindow",
      "THIS", "WebGPU::Direct::SurfaceDescriptorFromXlibWindow");
  }

  HV *h = SvRV(THIS);
  WGPUSurfaceDescriptorFromXlibWindow *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSurfaceDescriptorFromXlibWindow);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "chain", 5, &n->chain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "display", 7, &n->display, newSVpvs("void"));
  _find_set_uint32_t(aTHX_ h, "window", 6, &n->window, newSVpvs("uint32_t"));

}

void WGPUSwapChainDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SwapChainDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SwapChainDescriptor",
      "THIS", "WebGPU::Direct::SwapChainDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUSwapChainDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSwapChainDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_WGPUFlags(aTHX_ h, "usage", 5, &n->usage, newSVpvs("WGPUTextureUsageFlags"));
  _find_set_enum(aTHX_ h, "format", 6, &n->format, newSVpvs("WGPUTextureFormat"));
  _find_set_uint32_t(aTHX_ h, "width", 5, &n->width, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "height", 6, &n->height, newSVpvs("uint32_t"));
  _find_set_enum(aTHX_ h, "presentMode", 11, &n->presentMode, newSVpvs("WGPUPresentMode"));

}

void WGPUTextureBindingLayout__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::TextureBindingLayout"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::TextureBindingLayout",
      "THIS", "WebGPU::Direct::TextureBindingLayout");
  }

  HV *h = SvRV(THIS);
  WGPUTextureBindingLayout *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUTextureBindingLayout);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_enum(aTHX_ h, "sampleType", 10, &n->sampleType, newSVpvs("WGPUTextureSampleType"));
  _find_set_enum(aTHX_ h, "viewDimension", 13, &n->viewDimension, newSVpvs("WGPUTextureViewDimension"));
  _find_set_bool(aTHX_ h, "multisampled", 12, &n->multisampled, newSVpvs("bool"));

}

void WGPUTextureDataLayout__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::TextureDataLayout"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::TextureDataLayout",
      "THIS", "WebGPU::Direct::TextureDataLayout");
  }

  HV *h = SvRV(THIS);
  WGPUTextureDataLayout *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUTextureDataLayout);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_uint64_t(aTHX_ h, "offset", 6, &n->offset, newSVpvs("uint64_t"));
  _find_set_uint32_t(aTHX_ h, "bytesPerRow", 11, &n->bytesPerRow, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "rowsPerImage", 12, &n->rowsPerImage, newSVpvs("uint32_t"));

}

void WGPUTextureViewDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::TextureViewDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::TextureViewDescriptor",
      "THIS", "WebGPU::Direct::TextureViewDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUTextureViewDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUTextureViewDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_enum(aTHX_ h, "format", 6, &n->format, newSVpvs("WGPUTextureFormat"));
  _find_set_enum(aTHX_ h, "dimension", 9, &n->dimension, newSVpvs("WGPUTextureViewDimension"));
  _find_set_uint32_t(aTHX_ h, "baseMipLevel", 12, &n->baseMipLevel, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "mipLevelCount", 13, &n->mipLevelCount, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "baseArrayLayer", 14, &n->baseArrayLayer, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "arrayLayerCount", 15, &n->arrayLayerCount, newSVpvs("uint32_t"));
  _find_set_enum(aTHX_ h, "aspect", 6, &n->aspect, newSVpvs("WGPUTextureAspect"));

}

void WGPUVertexAttribute__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::VertexAttribute"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::VertexAttribute",
      "THIS", "WebGPU::Direct::VertexAttribute");
  }

  HV *h = SvRV(THIS);
  WGPUVertexAttribute *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUVertexAttribute);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_enum(aTHX_ h, "format", 6, &n->format, newSVpvs("WGPUVertexFormat"));
  _find_set_uint64_t(aTHX_ h, "offset", 6, &n->offset, newSVpvs("uint64_t"));
  _find_set_uint32_t(aTHX_ h, "shaderLocation", 14, &n->shaderLocation, newSVpvs("uint32_t"));

}

void WGPUBindGroupDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::BindGroupDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::BindGroupDescriptor",
      "THIS", "WebGPU::Direct::BindGroupDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUBindGroupDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUBindGroupDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_void(aTHX_ h, "layout", 6, &n->layout, newSVpvs("WGPUBindGroupLayout"));
  _find_set_uint32_t(aTHX_ h, "entryCount", 10, &n->entryCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "entries", 7, &n->entries, newSVpvs("WebGPU::Direct::BindGroupEntry"));

}

void WGPUBindGroupLayoutEntry__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::BindGroupLayoutEntry"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::BindGroupLayoutEntry",
      "THIS", "WebGPU::Direct::BindGroupLayoutEntry");
  }

  HV *h = SvRV(THIS);
  WGPUBindGroupLayoutEntry *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUBindGroupLayoutEntry);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_uint32_t(aTHX_ h, "binding", 7, &n->binding, newSVpvs("uint32_t"));
  _find_set_WGPUFlags(aTHX_ h, "visibility", 10, &n->visibility, newSVpvs("WGPUShaderStageFlags"));
  _find_set_obj(aTHX_ h, "buffer", 6, &n->buffer, newSVpvs("WebGPU::Direct::BufferBindingLayout"));
  _find_set_obj(aTHX_ h, "sampler", 7, &n->sampler, newSVpvs("WebGPU::Direct::SamplerBindingLayout"));
  _find_set_obj(aTHX_ h, "texture", 7, &n->texture, newSVpvs("WebGPU::Direct::TextureBindingLayout"));
  _find_set_obj(aTHX_ h, "storageTexture", 14, &n->storageTexture, newSVpvs("WebGPU::Direct::StorageTextureBindingLayout"));

}

void WGPUBlendState__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::BlendState"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::BlendState",
      "THIS", "WebGPU::Direct::BlendState");
  }

  HV *h = SvRV(THIS);
  WGPUBlendState *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUBlendState);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "color", 5, &n->color, newSVpvs("WebGPU::Direct::BlendComponent"));
  _find_set_obj(aTHX_ h, "alpha", 5, &n->alpha, newSVpvs("WebGPU::Direct::BlendComponent"));

}

void WGPUCompilationInfo__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::CompilationInfo"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::CompilationInfo",
      "THIS", "WebGPU::Direct::CompilationInfo");
  }

  HV *h = SvRV(THIS);
  WGPUCompilationInfo *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUCompilationInfo);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_uint32_t(aTHX_ h, "messageCount", 12, &n->messageCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "messages", 8, &n->messages, newSVpvs("WebGPU::Direct::CompilationMessage"));

}

void WGPUComputePassDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ComputePassDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ComputePassDescriptor",
      "THIS", "WebGPU::Direct::ComputePassDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUComputePassDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUComputePassDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "timestampWriteCount", 19, &n->timestampWriteCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "timestampWrites", 15, &n->timestampWrites, newSVpvs("WebGPU::Direct::ComputePassTimestampWrite"));

}

void WGPUDepthStencilState__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::DepthStencilState"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::DepthStencilState",
      "THIS", "WebGPU::Direct::DepthStencilState");
  }

  HV *h = SvRV(THIS);
  WGPUDepthStencilState *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUDepthStencilState);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_enum(aTHX_ h, "format", 6, &n->format, newSVpvs("WGPUTextureFormat"));
  _find_set_bool(aTHX_ h, "depthWriteEnabled", 17, &n->depthWriteEnabled, newSVpvs("bool"));
  _find_set_enum(aTHX_ h, "depthCompare", 12, &n->depthCompare, newSVpvs("WGPUCompareFunction"));
  _find_set_obj(aTHX_ h, "stencilFront", 12, &n->stencilFront, newSVpvs("WebGPU::Direct::StencilFaceState"));
  _find_set_obj(aTHX_ h, "stencilBack", 11, &n->stencilBack, newSVpvs("WebGPU::Direct::StencilFaceState"));
  _find_set_uint32_t(aTHX_ h, "stencilReadMask", 15, &n->stencilReadMask, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "stencilWriteMask", 16, &n->stencilWriteMask, newSVpvs("uint32_t"));
  _find_set_int32_t(aTHX_ h, "depthBias", 9, &n->depthBias, newSVpvs("int32_t"));
  _find_set_float(aTHX_ h, "depthBiasSlopeScale", 19, &n->depthBiasSlopeScale, newSVpvs("float"));
  _find_set_float(aTHX_ h, "depthBiasClamp", 14, &n->depthBiasClamp, newSVpvs("float"));

}

void WGPUImageCopyBuffer__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ImageCopyBuffer"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ImageCopyBuffer",
      "THIS", "WebGPU::Direct::ImageCopyBuffer");
  }

  HV *h = SvRV(THIS);
  WGPUImageCopyBuffer *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUImageCopyBuffer);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_obj(aTHX_ h, "layout", 6, &n->layout, newSVpvs("WebGPU::Direct::TextureDataLayout"));
  _find_set_void(aTHX_ h, "buffer", 6, &n->buffer, newSVpvs("WGPUBuffer"));

}

void WGPUImageCopyTexture__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ImageCopyTexture"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ImageCopyTexture",
      "THIS", "WebGPU::Direct::ImageCopyTexture");
  }

  HV *h = SvRV(THIS);
  WGPUImageCopyTexture *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUImageCopyTexture);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "texture", 7, &n->texture, newSVpvs("WGPUTexture"));
  _find_set_uint32_t(aTHX_ h, "mipLevel", 8, &n->mipLevel, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "origin", 6, &n->origin, newSVpvs("WebGPU::Direct::Origin3D"));
  _find_set_enum(aTHX_ h, "aspect", 6, &n->aspect, newSVpvs("WGPUTextureAspect"));

}

void WGPUProgrammableStageDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ProgrammableStageDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ProgrammableStageDescriptor",
      "THIS", "WebGPU::Direct::ProgrammableStageDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUProgrammableStageDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUProgrammableStageDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "module", 6, &n->module, newSVpvs("WGPUShaderModule"));
  _find_set_str(aTHX_ h, "entryPoint", 10, &n->entryPoint, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "antCount", 8, &n->antCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "constants", 9, &n->constants, newSVpvs("WebGPU::Direct::ConstantEntry"));

}

void WGPURenderPassColorAttachment__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RenderPassColorAttachment"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RenderPassColorAttachment",
      "THIS", "WebGPU::Direct::RenderPassColorAttachment");
  }

  HV *h = SvRV(THIS);
  WGPURenderPassColorAttachment *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURenderPassColorAttachment);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_void(aTHX_ h, "view", 4, &n->view, newSVpvs("WGPUTextureView"));
  _find_set_void(aTHX_ h, "resolveTarget", 13, &n->resolveTarget, newSVpvs("WGPUTextureView"));
  _find_set_enum(aTHX_ h, "loadOp", 6, &n->loadOp, newSVpvs("WGPULoadOp"));
  _find_set_enum(aTHX_ h, "storeOp", 7, &n->storeOp, newSVpvs("WGPUStoreOp"));
  _find_set_obj(aTHX_ h, "clearValue", 10, &n->clearValue, newSVpvs("WebGPU::Direct::Color"));

}

void WGPURequiredLimits__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RequiredLimits"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RequiredLimits",
      "THIS", "WebGPU::Direct::RequiredLimits");
  }

  HV *h = SvRV(THIS);
  WGPURequiredLimits *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURequiredLimits);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_obj(aTHX_ h, "limits", 6, &n->limits, newSVpvs("WebGPU::Direct::Limits"));

}

void WGPUShaderModuleDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ShaderModuleDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ShaderModuleDescriptor",
      "THIS", "WebGPU::Direct::ShaderModuleDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUShaderModuleDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUShaderModuleDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "hintCount", 9, &n->hintCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "hints", 5, &n->hints, newSVpvs("WebGPU::Direct::ShaderModuleCompilationHint"));

}

void WGPUSupportedLimits__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::SupportedLimits"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::SupportedLimits",
      "THIS", "WebGPU::Direct::SupportedLimits");
  }

  HV *h = SvRV(THIS);
  WGPUSupportedLimits *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUSupportedLimits);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStructOut"));
  _find_set_obj(aTHX_ h, "limits", 6, &n->limits, newSVpvs("WebGPU::Direct::Limits"));

}

void WGPUTextureDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::TextureDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::TextureDescriptor",
      "THIS", "WebGPU::Direct::TextureDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUTextureDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUTextureDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_WGPUFlags(aTHX_ h, "usage", 5, &n->usage, newSVpvs("WGPUTextureUsageFlags"));
  _find_set_enum(aTHX_ h, "dimension", 9, &n->dimension, newSVpvs("WGPUTextureDimension"));
  _find_set_obj(aTHX_ h, "size", 4, &n->size, newSVpvs("WebGPU::Direct::Extent3D"));
  _find_set_enum(aTHX_ h, "format", 6, &n->format, newSVpvs("WGPUTextureFormat"));
  _find_set_uint32_t(aTHX_ h, "mipLevelCount", 13, &n->mipLevelCount, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "sampleCount", 11, &n->sampleCount, newSVpvs("uint32_t"));
  _find_set_uint32_t(aTHX_ h, "viewFormatCount", 15, &n->viewFormatCount, newSVpvs("uint32_t"));
  _find_set_enum(aTHX_ h, "viewFormats", 11, &n->viewFormats, newSVpvs("WGPUTextureFormat"));

}

void WGPUVertexBufferLayout__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::VertexBufferLayout"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::VertexBufferLayout",
      "THIS", "WebGPU::Direct::VertexBufferLayout");
  }

  HV *h = SvRV(THIS);
  WGPUVertexBufferLayout *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUVertexBufferLayout);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_uint64_t(aTHX_ h, "arrayStride", 11, &n->arrayStride, newSVpvs("uint64_t"));
  _find_set_enum(aTHX_ h, "stepMode", 8, &n->stepMode, newSVpvs("WGPUVertexStepMode"));
  _find_set_uint32_t(aTHX_ h, "attributeCount", 14, &n->attributeCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "attributes", 10, &n->attributes, newSVpvs("WebGPU::Direct::VertexAttribute"));

}

void WGPUBindGroupLayoutDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::BindGroupLayoutDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::BindGroupLayoutDescriptor",
      "THIS", "WebGPU::Direct::BindGroupLayoutDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUBindGroupLayoutDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUBindGroupLayoutDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "entryCount", 10, &n->entryCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "entries", 7, &n->entries, newSVpvs("WebGPU::Direct::BindGroupLayoutEntry"));

}

void WGPUColorTargetState__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ColorTargetState"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ColorTargetState",
      "THIS", "WebGPU::Direct::ColorTargetState");
  }

  HV *h = SvRV(THIS);
  WGPUColorTargetState *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUColorTargetState);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_enum(aTHX_ h, "format", 6, &n->format, newSVpvs("WGPUTextureFormat"));
  _find_set_obj(aTHX_ h, "blend", 5, &n->blend, newSVpvs("WebGPU::Direct::BlendState"));
  _find_set_WGPUFlags(aTHX_ h, "writeMask", 9, &n->writeMask, newSVpvs("WGPUColorWriteMaskFlags"));

}

void WGPUComputePipelineDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::ComputePipelineDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::ComputePipelineDescriptor",
      "THIS", "WebGPU::Direct::ComputePipelineDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUComputePipelineDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUComputePipelineDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_void(aTHX_ h, "layout", 6, &n->layout, newSVpvs("WGPUPipelineLayout"));
  _find_set_obj(aTHX_ h, "compute", 7, &n->compute, newSVpvs("WebGPU::Direct::ProgrammableStageDescriptor"));

}

void WGPUDeviceDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::DeviceDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::DeviceDescriptor",
      "THIS", "WebGPU::Direct::DeviceDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPUDeviceDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUDeviceDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "requiredFeaturesCount", 21, &n->requiredFeaturesCount, newSVpvs("uint32_t"));
  _find_set_enum(aTHX_ h, "requiredFeatures", 16, &n->requiredFeatures, newSVpvs("WGPUFeatureName"));
  _find_set_obj(aTHX_ h, "requiredLimits", 14, &n->requiredLimits, newSVpvs("WebGPU::Direct::RequiredLimits"));
  _find_set_obj(aTHX_ h, "defaultQueue", 12, &n->defaultQueue, newSVpvs("WebGPU::Direct::QueueDescriptor"));

}

void WGPURenderPassDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RenderPassDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RenderPassDescriptor",
      "THIS", "WebGPU::Direct::RenderPassDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPURenderPassDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURenderPassDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "colorAttachmentCount", 20, &n->colorAttachmentCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "colorAttachments", 16, &n->colorAttachments, newSVpvs("WebGPU::Direct::RenderPassColorAttachment"));
  _find_set_obj(aTHX_ h, "depthStencilAttachment", 22, &n->depthStencilAttachment, newSVpvs("WebGPU::Direct::RenderPassDepthStencilAttachment"));
  _find_set_void(aTHX_ h, "occlusionQuerySet", 17, &n->occlusionQuerySet, newSVpvs("WGPUQuerySet"));
  _find_set_uint32_t(aTHX_ h, "timestampWriteCount", 19, &n->timestampWriteCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "timestampWrites", 15, &n->timestampWrites, newSVpvs("WebGPU::Direct::RenderPassTimestampWrite"));

}

void WGPUVertexState__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::VertexState"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::VertexState",
      "THIS", "WebGPU::Direct::VertexState");
  }

  HV *h = SvRV(THIS);
  WGPUVertexState *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUVertexState);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "module", 6, &n->module, newSVpvs("WGPUShaderModule"));
  _find_set_str(aTHX_ h, "entryPoint", 10, &n->entryPoint, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "antCount", 8, &n->antCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "constants", 9, &n->constants, newSVpvs("WebGPU::Direct::ConstantEntry"));
  _find_set_uint32_t(aTHX_ h, "bufferCount", 11, &n->bufferCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "buffers", 7, &n->buffers, newSVpvs("WebGPU::Direct::VertexBufferLayout"));

}

void WGPUFragmentState__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::FragmentState"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::FragmentState",
      "THIS", "WebGPU::Direct::FragmentState");
  }

  HV *h = SvRV(THIS);
  WGPUFragmentState *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPUFragmentState);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_void(aTHX_ h, "module", 6, &n->module, newSVpvs("WGPUShaderModule"));
  _find_set_str(aTHX_ h, "entryPoint", 10, &n->entryPoint, newSVpvs("char"));
  _find_set_uint32_t(aTHX_ h, "antCount", 8, &n->antCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "constants", 9, &n->constants, newSVpvs("WebGPU::Direct::ConstantEntry"));
  _find_set_uint32_t(aTHX_ h, "targetCount", 11, &n->targetCount, newSVpvs("uint32_t"));
  _find_set_obj(aTHX_ h, "targets", 7, &n->targets, newSVpvs("WebGPU::Direct::ColorTargetState"));

}

void WGPURenderPipelineDescriptor__pack( SV *THIS )
{
  if (!SvROK(THIS) || !sv_derived_from(THIS, "WebGPU::Direct::RenderPipelineDescriptor"))
  {
    croak_nocontext("%s: %s is not of type %s",
      "WebGPU::Direct::RenderPipelineDescriptor",
      "THIS", "WebGPU::Direct::RenderPipelineDescriptor");
  }

  HV *h = SvRV(THIS);
  WGPURenderPipelineDescriptor *n = _get_struct_ptr(aTHX, THIS, NULL);
  if ( !n )
  {
    Newxz(n, 1, WGPURenderPipelineDescriptor);
    sv_magicext(h, NULL, PERL_MAGIC_ext, NULL, (const char *)n, 0);
  }
  SV **f;
  SV *fp;
  _find_set_obj(aTHX_ h, "nextInChain", 11, &n->nextInChain, newSVpvs("WebGPU::Direct::ChainedStruct"));
  _find_set_str(aTHX_ h, "label", 5, &n->label, newSVpvs("char"));
  _find_set_void(aTHX_ h, "layout", 6, &n->layout, newSVpvs("WGPUPipelineLayout"));
  _find_set_obj(aTHX_ h, "vertex", 6, &n->vertex, newSVpvs("WebGPU::Direct::VertexState"));
  _find_set_obj(aTHX_ h, "primitive", 9, &n->primitive, newSVpvs("WebGPU::Direct::PrimitiveState"));
  _find_set_obj(aTHX_ h, "depthStencil", 12, &n->depthStencil, newSVpvs("WebGPU::Direct::DepthStencilState"));
  _find_set_obj(aTHX_ h, "multisample", 11, &n->multisample, newSVpvs("WebGPU::Direct::MultisampleState"));
  _find_set_obj(aTHX_ h, "fragment", 8, &n->fragment, newSVpvs("WebGPU::Direct::FragmentState"));

}

