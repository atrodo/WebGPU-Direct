...
void _set_callback(pTHX_ CV *callback, SV *userdata, void *cb_field, void *ud_field, WGPUProc cb_fn)
{
  // if ud_field is already valid, check if its the same data. If not, free and refdec it before setting to the new value
  if ( ud_field != NULL )
  {
    cb_data *ud = (cb_data *)ud_field;
    if ( ud->guard1 == CB_GUARD && ud->guard2 == ud->guard1 )
    {
      // Valid userdata, we can check it

      // If it is identical data, jobs done
      if ( ud->perlsub == callback && ud->data == userdata )
      {
        return;
      }

      // Otherwise we need to free the resources
      SvREFCNT_dec(ud->perlsub);
      SvREFCNT_dec(ud->data);
      Safefree(ud);
    }
  }

  // At this point, ud_field is either null or effectively null
  cb_data *ud;
  Newx(ud, 1, cb_data);
  *ud = (cb_data) {
    .guard1 = CB_GUARD,
...
SV *_unpack_callback(pTHX_ HV *h, const char *cb_key, I32 cb_klen, const char *ud_key, I32 ud_klen, void *cb_field, void *ud_field, WGPUProc cb_fn)
{
  SV **cb_f;
  SV **ud_f;
  ...
  // Find the field from the hash
  cb_f = hv_fetch(h, cb_key, cb_klen, 1);
  ud_f = hv_fetch(h, ud_key, ud_klen, 1);

  if ( !( cb_f && *cb_f ) )
  {
    croak("Could not save new callback value for %s", cb_key);
  }

  if ( !( ud_f && *ud_f ) )
  {
    croak("Could not save new userdata value for %s", ud_key);
  }
  ...
  if ( ud == NULL )
  {
    _set_callback(aTHX_ (CV *)*cb_f, *ud_f, cb_field, ud_field, cb_fn);
    cb_data *ud = (cb_data *)ud_field;
  }

  if ( !( ud->guard1 == CB_GUARD && ud->guard2 == ud->guard1 ) )
  {
    croak("Got a callback with improper guards: 0x%X, 0x%X", ud->guard1, ud->guard2);
  }

  cb_f = nn_hv_store(aTHX_ h, cb_key, cb_klen, (SV *)ud->perlsub, &PL_sv_undef);
  ud_f = nn_hv_store(aTHX_ h, ud_key, ud_klen, (SV *)ud->data, &PL_sv_undef);

  return *ud_f;
}

SV *_pack_callback(pTHX_ HV *h, const char *cb_key, I32 cb_klen, const char *ud_key, I32 ud_klen, void *cb_field, void *ud_field, WGPUProc cb_fn)
{
  SV **cb_f;
  SV **ud_f;
  ...
  // Find the field from the hash
  cb_f = hv_fetch(h, cb_key, cb_klen, 0);
  ud_f = hv_fetch(h, ud_key, ud_klen, 0);

  // If either field is not found, create a default one
  if ( !( cb_f && *cb_f ) )
  {
    cb_f = nn_hv_store(aTHX_ h, cb_key, cb_klen, &PL_sv_undef, &PL_sv_undef);
  }

  if ( !( ud_f && *ud_f ) )
  {
    ud_f = nn_hv_store(aTHX_ h, ud_key, ud_klen, &PL_sv_undef, &PL_sv_undef);
  }

  if ( SvTYPE(*cb_f) != SVt_NULL )
  {
    if ( !SvROK(*cb_f) || SvTYPE(SvRV(*cb_f)) != SVt_PVCV )
    {
      croak("The callback field must be a code block, but got '%s'", SvPV_nolen(*cb_f));
    }
  }
...
SV *_find_callback(pTHX_ HV *h, const char *key, I32 klen, void *field, SV *base)
{
  croak("...%d", __LINE__);
}

void _store_callback(pTHX_ HV *h, const char *cb_key, I32 cb_klen, const char *ud_key, I32 ud_klen, void *cb_field, void *ud_field, CV *callback, SV *userdata)
{
  croak("...%d", __LINE__);
}
